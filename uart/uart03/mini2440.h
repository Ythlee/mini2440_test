...
/* 中断 基地址 */
#define INT_BASE	((volatile unsigned int *)0x4A000000)
...
typedef struct {
    __IO unsigned int 	SRCPND;        //Interrupt request status
    __IO unsigned int 	INTMOD;        //Interrupt mode control
    __IO unsigned int 	INTMSK;        //Interrupt mask control
    __IO unsigned int 	PRIORITY;      //IRQ priority control
    __IO unsigned int 	INTPND;        //Interrupt request status
    __IO unsigned int 	INTOFFSET;     //Interrupt request source offset
    __IO unsigned int 	SUBSRCPND;     //Sub source pending
    __IO unsigned int 	INTSUBMSK;     //Interrupt sub mask
} INT_TypeDef;
